...
$orders = $orderDB->getByStatus("Shipping");

if (isset($_GET['success'])) {
    $order_id = $_GET['success'];
    $orderDB->updateStatus("Success", $order_id);
    header("Location: order-ship.php");
}
                                ...
                                <table class="table table-bordered table-hover dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Office: activate to sort column ascending" style="width:220px">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php foreach ($orders as $order) : ?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="order-ship.php?success=<?= $order['order_id'] ?>" class="btn btn-primary">Duyệt</a>
                                                    <a href="display-order.php?order_id=<?= $order['order_id'] ?>&product_id=<?= $order['order_id'] ?>" class="btn btn-danger">Delete</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
