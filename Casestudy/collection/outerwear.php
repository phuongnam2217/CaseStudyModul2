    ...
    <div class="grid">
        <div class="layout-arrival">
            ...
            <div class="arrival-product">
                <?php foreach ($productOuterwear as $value) : ?>
                    <div class="col-md-3 col-sm-6 col-xs-6 product-column">
                        <div class="product-top">
                            ...
                            <div class="product-img-btn" id="img-btn">
                                <a href="outerwear.php?id=<?= $value['product_id'] ?>" class="ajax-cart">Add to card</a>
                            </div>
                        </div>
            ...
            <div class="pagination">
                <?php if ($current_page > 1 && $total_page > 1) : ?>
                    <a class="btn btn-outline-primary pagination-btn" href="outerwear.php?page=<?= $current_page - 1 ?>">Prev</a>
                <?php endif; ?>
                <?php for ($i = 1; $i <= $total_page; $i++) : ?>
                    <?php if ($i == $current_page) : ?>
                        <span><a class="btn btn-outline-primary pagination-btn" href="outerwear.php?page=<?= $i ?>"><?= $i ?></a></span>
                    <?php else : ?>
                        <a class="btn btn-outline-primary pagination-btn" href="outerwear.php?page=<?= $i ?>"><?= $i ?></a>
                    <?php endif; ?>
                <?php endfor; ?>
                <?php if ($current_page < $total_page && $total_page > 1) : ?>
                    <a class="btn btn-outline-primary pagination-btn" href="outerwear.php?page=<?= $current_page + 1 ?>">Next</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
